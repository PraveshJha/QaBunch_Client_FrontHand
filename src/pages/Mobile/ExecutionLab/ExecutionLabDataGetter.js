...
export class ExecutionLabDataGetter {
  ...
  async renderEnvironment(allconfigData) {
    ...
    ExecutionLabData.SelectedScreenOption = allconfigData.DefaultExecutionPlatform;
    await this.setDeviceInforMation(await allconfigData.DefaultExecutionPlatform);
  }
  ...
  async executeUITestScripts(environment, runAt, threadCount, reportInDashBoard, screenName, deviceName, screenShotOption, selectedScripts) {
    ...
    if (Config.isDemo) {
      ...
      var headers = { 'Authorization': await Users.userToken, userEmail: await Users.userEmail };
      var serverResponse = await restAPI.post(backendAPI + 'mobileexecutor/project/' + selectedProject, await headers, await executionParams);
  ...
  async createTestSuiteForSpecificComponent(componentName, rowId = 1) {
    ...
    if (Config.isDemo) {
      ...
      var selectedProject = await localStorage.getItem('UserSelectedAccount');
      var allComponentTestDetails = await GetData.getListOfTestIdAndTestName(selectedProject, 'Mobile', componentName);
  ...
  async setDeviceInforMation(platform) {
    if (Config.isDemo) {
      switch (platform) {
        case "Android":
          ExecutionLabData.DeviceList = ['One plus 9R', 'Pixel 3'];
          ExecutionLabData.SelectedDevice = 'Pixel 3';
          break;
        case "iOS":
          ExecutionLabData.DeviceList = ['iPhone 12', 'iPhone 14'];
          ExecutionLabData.SelectedDevice = 'iPhone 14';
          break;
        default:
          ExecutionLabData.DeviceList = [];
      ...
      var allConFigData = ExecutionLabData.AllConfigData;
      var allDevice =[];
       ...
       {
        ExecutionLabData.DeviceList = await allDevice;
        ExecutionLabData.SelectedDevice = await allDevice[0];
       }
       else{
        ExecutionLabData.DeviceList = [];
          ExecutionLabData.SelectedDevice = '';
